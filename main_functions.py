...
def loading_environments():
    ...
    env_param = {}
    path_rewards_0_1 = 'Environments/Rewards_-0.1.npy'
    malus_0_1 = np.load(path_rewards_0_1, allow_pickle=True)
    path_transitions_0_1 = 'Environments/Transitions_-0.1_1.npy'
    transitions_0_1 = np.load(path_transitions_0_1, allow_pickle=True)
    env_param['Lopes'] = {'transitions': transitions_0_1,
                          'rewards': malus_0_1}

    for index_non_stat in range(1, 21):

        transitions_non_stat_article = np.load(
            'Environments/Transitions_non_stat_article_-0.1_1_' +
            str(index_non_stat) + '.npy',
            allow_pickle=True)

        transitions_strong_non_stat = np.load(
            'Environments/Transitions_strong_non_stat_-0.1_1_' +
            str(index_non_stat)+'.npy',
            allow_pickle=True)

        env_param["Lopes_non_stat_article_{0}".format(index_non_stat)] = {
            'transitions': transitions_0_1,
            'rewards': malus_0_1,
    ...
    for nb_env in range(1, 11):
        transitions_1 = np.load(
            'Environments/Transitions_-1_' + str(nb_env) + '.npy',
            allow_pickle=True)
...
def play(environment,
    ...
    """
    agents = {'R-max': Rmax, 'ζ-R-max': RmaxLP,
              'BEB': BEB, 'ζ-EB': EBLP, 'ε-greedy': EpsilonMB}
    agent = agents[name_agent](environment, **agent_parameters)

    reward_per_episode = []
    optimal_policy_value_error = []
    real_policy_value_error = []
    ...
    for trial in range(trials):
        ...
        while not game_over:
            if environment.total_steps == 900:
                val_iteration, _ = value_iteration(
                    environment, agent.gamma, accuracy_VI)
            if agent.step_counter % step_between_VI == 0:
                ...
                optimal_policy_value_error.append(
                    optimal_policy_value_s0 - best_policy_value_s0)
                ...
                real_policy_value_error.append(
                    real_policy_value_s0-best_policy_value_s0)
...
def main_function(all_seeds, every_simulation, play_params, agent_parameters):
    ...
    if type(agent_parameters) == dict:
        all_parameters = [[play_params, all_seeds[index_seed],
                           agent_parameters,
                           every_simulation[index_seed]]
                          for index_seed in range(len(all_seeds))]
    else:
        all_parameters = [[play_params, all_seeds[index_seed],
                           agent_parameters[index_seed],
                           every_simulation[index_seed]]
                          for index_seed in range(len(all_seeds))]
    nb_processes_to_use = max(1, cpu_count()-1)
    pool = Pool(processes=nb_processes_to_use)
    ...
    print('Computation time: '+str(round(time_after - before))+" seconds.")
    return opt_pol_errors, real_pol_errors, rewards


def play_with_params(play_parameters,
                     seed,
                     agent_parameters,
                     simulation_to_do):
    np.random.seed(seed)
    ...
    return simulation_to_do, play(environment,
                                  name_agent,
                                  agent_parameters[name_agent],
                                  **play_parameters)
...
def save_and_plot(optimal_stats, real_stats, rewards_stats, agents_tested,
    ...
    np.save('Data/OptimalPolicy/'+time_end+'_polerror_opti.npy', pol_opti)
    np.save('Data/AgentPolicy/'+time_end+'_polerror_real.npy', pol_real)
    np.save('Data/Rewards/'+time_end+'_rewards.npy', reward)

    colors = {'R-max': '#9d02d7', 'ζ-R-max': '#0000ff',
              'ε-greedy': "#ff7763", 'BEB': "#ffac1e", 'ζ-EB': "#009435"}
    marker_sizes = {'R-max': '2', 'ζ-R-max': '2',
                    'BEB': '2', 'ζ-EB': '2', 'ε-greedy': '2'}
    markers = {'R-max': '^', 'ζ-R-max': 'o',
               'BEB': 'x', 'ζ-EB': '*', 'ε-greedy': 's'}
    ...
    plot_agents(agents_tested, pol_opti, SEM_pol_opti,
...
def plot_agents(agents_tested, values, SEM_values, x_range, colors, markers, marker_sizes):
    ...
    for name_agent in agents_tested:
        ...
        plt.fill_between(x_range, yerr0, yerr1,
                         color=colors[name_agent], alpha=0.2)
...
def take_picture(agent, trial, environment, photos):
    ...
    if trial in photos:
        ...
        if type(agent).__name__ in ['BEB', 'Rmax', 'EBLP', 'RmaxLP']:
            ...
            best_values_bonus, best_policy_bonus = get_max_Q_values_and_policy(
                bonus)
            plot_V(best_values_bonus, best_policy_bonus, 122)
        plt.savefig("ValueIterationPolicies/"+type(agent).__name__+"_"+type(environment).__name__ +
...
def compute_optimal_policies():
    ...
    for name_env in env_param.keys():
        ...
        plt.figure(dpi=300)
        plot_V(V, policy, 111)
        plt.savefig("ValueIterationPolicies/OptimalPolicies/VI_" +
